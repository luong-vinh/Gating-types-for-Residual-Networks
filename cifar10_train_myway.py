...
# Just make a copy of cifar10_train here so that I can run 2 types 
# of network at the same time in my Titan Z
...
def setConfig():    
    ...
    config['train_dir'] = 'cifar10_train'
...
#                                                      numStackedBlocks = 9)
    ...
    print('Gonna run with adaptive learning rate')
    # Vinh: awkward way to change learning rate here
    lr = tf.placeholder(tf.float32, [], "learning_rate")
    
    
    optimizer = tf.train.AdamOptimizer(epsilon = 0.1)
    # TODO: will fix this a bit later    
#     train_op = cifar10.train2(loss, global_step, optimizer)
    ...
    for step in xrange(max_steps):
      ...
      # Not reducing the learning rate for now: ResNet's paper
      if (step + 1) == 32000:
        feed_dict = {lr : 0.01}
      elif (step + 1) == 48000:
        feed_dict = {lr : 0.001}
